...
public class UDLoggingInInterceptor extends LoggingInInterceptor {
  ...
  private static String headerBlacklist;
    ...
    if (!isShowBinaryContent() && isBinaryContent(contentType)) {
      ...
      auditLogService.createUDAuditRecord(
          UDAuditLogBuilder.createAuditLog(
              "UDLoggingInInterceptor", timestampGenerator.getCurrentTimeStamp(), message));
    ...
    for (Map.Entry<String, String> securityField : securityData.entrySet()) {
      if (!securityAuditFields.contains(securityField.getKey())) {
        securityAuditLogRecord
            .append(securityField.getKey())
            .append("=")
            .append(securityField.getValue())
            .append("|");
      }
    }
  ...
  public void setDisplayPayload(boolean displayPayload) {
