...
public class KeystoreFactory {
  ...
  public static KeyStore getKeystore(String keystoreFilePath, String password) {
    ...
    try {
      ...
      String msg =
          "Exception when loading Keystore: "
              + String.valueOf(keystoreFilePath)
              + "\n"
  ...
  private static Key getPrivateKey(KeyStore keyStore, String alias, String password)
    ...
    if (key == null) {
      throw new UnrecoverableKeyException(
          "Could not load key with alias: " + alias + " from keystore");
    }
    return key;
  }
  ...
  public static String getFieldValueFromCertificateSubjectDN(
      String targetFieldName, X509Certificate certificate)
      throws InterchangeSenderPermissionDeniedException {
    ...
    throw new InterchangeSenderPermissionDeniedException(
        "Could not match certificate alias using subject:" + subjectDN);
  }
}
