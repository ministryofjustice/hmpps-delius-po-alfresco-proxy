...
public class AlfrescoProxyHealthChecker {
  ...
  @Inject
  public AlfrescoProxyHealthChecker(
      ...
      @Named("AlfrescoHealthChecker")
          final DependencyHealthChecker<AlfrescoHealth> alfrescoHealthChecker,
      @Named("ClamAvHealthChecker")
          final DependencyHealthChecker<ClamAvHealth> clamAvHealthChecker) {
  ...
  public String checkHealth() throws JsonProcessingException {
    ...
    final String stability =
        Arrays.asList(alfrescoHealth.getStatus(), clamAvHealth.getStatus()).contains(FAULT)
            ? "UNSTABLE"
            : "STABLE";

    final ApiHealth apiHealth =
        new ApiHealth(
            applicationName,
            stability,
            unmodifiableMap(dependencies),
