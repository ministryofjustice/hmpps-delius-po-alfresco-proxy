...
public class OAuthRequestFilter extends AbstractAuthFilter implements ContainerRequestFilter {
  ...
  public void filter(ContainerRequestContext context) {
    ...
    try {
      ...
      for (Parameter parameter : listOfOauthParams) {
        log.info(
            "DEBUG*** oauthparam from decodeAuthorisation: "
                + parameter.getKey()
                + " = "
                + parameter.getValue());
      }
      ...
      // we also need to mix and match psn address vs non psn addresses, the spg is expecting
      // signatures signed against
      ...
      OAuthConsumer consumer =
          new OAuthConsumer(null, oauthMessage.getConsumerKey(), null, serviceProvider);
      ...
      String commonName =
          getFieldValueFromCertificateSubjectDN("CN", (X509Certificate) certificate);
      ...
      if (proxyUrlRewritePattern != null && proxyUrlRewritePattern.length() > 0) {
        // note this is hard to test in the current spg-all-200 docker deploy, as it doesn't like
        // subdomains for some reason
        ...
        log.info(
            "Intialising oauth again with new url, as had to load it earlier to determine cert details");
        oauthMessage = new OAuthMessage(req.getMethod(), requestURL, listOfOauthParams);
      ...
      if (!Arrays.asList(permittedSenderIds).contains(senderId)) {
        ...
        throw new InterchangeSenderPermissionDeniedException(
            "Certificate with common name: "
                + commonName
                + " is not permitted to send as sender id: "
                + senderId
                + ". Permitted sender ids are: "
                + Arrays.toString(permittedSenderIds));
      }
      ...
      auditLogService.createUDAuditRecord(
          UDAuditLogBuilder.createAuditLog(
              UDSPGLogTypes.LOG_PROXY_UD_ABSORBED_MESSAGE.toString(),
  ...
  private String getProxyUrlRewritePattern(String commonName) {
    String proxyConnectionType =
        propertyResolver.getProperty(CALLING_VIA_PROXY_URL_REWRITE + commonName);
    log.info(
        commonName
            + " if present replaces first with second eg (spgw-ext.probation|spgw-int-psn.pre-prod.probation): "
            + proxyConnectionType);
  ...
  private String[] getPermittedSenderIds(String commonName) {
    ...
    String senderIdsString =
        propertyResolver.getProperty(CERTIFICATE_CN_TO_SENDER_ID_CFG_PREFIX + commonName);
