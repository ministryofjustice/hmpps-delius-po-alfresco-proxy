...
public class AlfrescoHealthCheckerTest {
  ...
  private static final WireMockServer WIREMOCK_SERVER =
      new WireMockServer(options().port(PORT).notifier(new ConsoleNotifier(true)));

  private final AlfrescoHealthChecker sut =
      new AlfrescoHealthChecker(alfrescoBaseUrl, alfrescoHealthCheckPath);
  ...
  public void testGetAlfrescoHealth() {
    WIREMOCK_SERVER.stubFor(
        get(urlEqualTo(alfrescoHealthCheckPath)).willReturn(aResponse().withStatus(200)));
  ...
  public void testGetAlfrescoHealthWhenStatusCodeIsNot200() {
    WIREMOCK_SERVER.stubFor(
        get(urlEqualTo(alfrescoHealthCheckPath)).willReturn(aResponse().withStatus(404)));
  ...
  public void testGetAlfrescoHealthWhenAlfrescoIsNotAvailable() {
    WIREMOCK_SERVER.stubFor(
        get(urlEqualTo(alfrescoHealthCheckPath)).willReturn(aResponse().withFault(EMPTY_RESPONSE)));

    final AlfrescoHealth expectedResult =
        new AlfrescoHealth(
            FAULT,
            0,
