...
public class AntivirusClientTest {
  ...
  private final GenericContainer clamAv =
      new GenericContainer<>(clamAvImage)
  ...
  private final ClamAvConnectionParametersProvider clamAvConnectionParametersProvider =
      mock(ClamAvConnectionParametersProvider.class);
  ...
  public void testGetClamAvVersion() {
    final ClamAvHealth expectedResult =
  ...
  public void testScanVirusCompromisedDocument() throws Exception {
    final Map<String, Collection<String>> viruses =
        singletonMap("stream", singleton("Win.Test.EICAR_HDB-1"));
    final AntivirusException antivirusException =
        new AntivirusException("Blacklisted: " + viruses + " FOUND");
    final AntivirusResponse expectedResponse =
        new AntivirusResponse(AntivirusResponse.Status.FAILED, antivirusException);
  ...
  public void testScanCleanDocument() throws Exception {
    final AntivirusResponse expectedResponse =
        new AntivirusResponse(AntivirusResponse.Status.PASSED);
  ...
  public void testScanDocumentWhenClamAvIsNotReachable() throws Exception {
    stopClamAV();
    final AntivirusException antivirusException =
        new AntivirusException("Could not connect to ClamAV");
    final AntivirusResponse expectedResponse =
        new AntivirusResponse(AntivirusResponse.Status.ERROR, antivirusException);
    final InputStream fileAsStream = fileReader.getFileAsStream("documents/no-virus.txt");
