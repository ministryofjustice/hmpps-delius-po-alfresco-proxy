...
public class AntiVirusSteps extends AbstractSteps implements En {
  ...
  public AntiVirusSteps() {
    Given(
        "^I have a virus compromised document \"([^\"]*)\" to upload$",
    ...
    When(
        "^I call \"([^\"]*)\" to upload the document$",
        (final String path) -> {
          this.uploadPath = path;

          final FileDataBodyPart filePart = new FileDataBodyPart("filedata", file);
          final MultiPart multiPart =
              new FormDataMultiPart().field("CRN", "X030927").bodyPart(filePart);
    ...
    Then(
        "^I should receive a response with status code \"([^\"]*)\"$",
