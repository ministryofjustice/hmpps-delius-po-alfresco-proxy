...
public class HealthCheckSteps extends AbstractSteps implements En {
  ...
  public HealthCheckSteps() {
    Given(
        "^the Alfresco Proxy API is running$",
        () -> {
          final String statusUp = "expectations/ping_response.json";
          final String jsonFile = jsonReader.readFile(statusUp);
          final JsonObject expectedPingResponse =
              JsonParser.parseString(jsonFile).getAsJsonObject();

          final Response response =
              webTarget.path("/").request(APPLICATION_JSON_TYPE).headers(headers).get();

          assertThat(response.getStatus(), is(200));
          assertThat(
              response.readEntity(Ping.class), is(gson.fromJson(expectedPingResponse, Ping.class)));
        });
    ...
    And(
        ...
        () -> {
          world
              .getWireMockServer()
              .stubFor(
                  get(urlEqualTo(alfrescoHealthEndpoint))
                      .willReturn(
                          aResponse()
                              .withStatus(200)
        ...
        () -> {
          ...
          alfrescoHealth =
              new AlfrescoHealth(
                  FAULT,
                  0,
                  "java.net.SocketException: SocketException invoking http://localhost:6067/afresco/s/admin-spg/healthcheck: Unexpected end of file from server");
        });

    When(
        "^I request the health of the Alfresco Proxy API$",
        () -> {
    ...
    Then(
        "^a response stating that the service is \"([^\"]*)\" is returned$",
        (String status) -> {
          clamAvHealth =
              clamAV.isRunning()
                  ...
                  : new ClamAvHealth(
                      FAULT,
                      "xyz.capybara.clamav.CommunicationException: Error while communicating with the server");
          ...
          assertThat(
              world.getResponse().getHeaders().get("Content-Type"), hasItem("application/json"));

          final String actualJsonBody = world.getResponse().readEntity(String.class);

          assertThatJson(actualJsonBody)
              .and(
